...
impl DashboardState {
    async fn new() -> Result<Self, GpuMonError> {
        // Load config and get database path
        let config = GpuMonConfig::load().unwrap_or_default();
    ...
    tracing_subscriber::registry()
        .with(
            EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| EnvFilter::new("info,gpumon_core=debug,gpumon_dashboard=debug"))
        )
