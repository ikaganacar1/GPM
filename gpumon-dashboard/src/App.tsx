...
function App() {
  ...
  useEffect(() => {
    async function fetchInfo() {
      try {
        const info: DashboardInfo = await invoke("get_dashboard_info");
  ...
  const fetchRealtimeMetrics = useCallback(async () => {
    try {
      const metrics: GpuMetricData[] = await invoke("get_realtime_metrics");
  ...
  const fetchChartData = useCallback(async () => {
    try {
      const data: ChartDataResponse = await invoke("get_chart_data", {
        gpuId: selectedGpu,
        hours: timeRange,
      });
    ...
    return (
      <div className="app-container">
        <div className="error">
          <h2>Error</h2>
          <p>{error}</p>
          <p>Make sure the gpumon service is running and collecting metrics.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="app-container">
      ...
      <footer className="app-footer">
        ...
        {dashboardInfo && (
          <span>Database: {dashboardInfo.database_path}</span>
        )}
      </footer>
    </div>
  );
