...
impl ParquetArchiver {
    ...
    fn read_from_sqlite(&self, _db_path: &Path, _query: &str) -> Result<DataFrame> {
        ...
        .map_err(|e| GpuMonError::ParquetError(format!("Failed to create DataFrame: {}", e)))?;
    ...
    fn write_parquet(&self, df: &DataFrame, path: &Path) -> Result<()> {
        ...
        ParquetWriter::new(file)
            ...
            .finish(&mut df.clone())
            .map_err(|e| GpuMonError::ParquetError(format!("Failed to write Parquet: {}", e)))?;
    ...
    pub fn read_parquet(&self, path: &Path) -> Result<DataFrame> {
        ...
        let df = ParquetReader::new(file)
            .finish()
            .map_err(|e| GpuMonError::ParquetError(format!("Failed to read Parquet: {}", e)))?;
