...
pub async fn start_server(port: u16, state: ApiState) -> Result<(), crate::error::GpuMonError> {
    let app = create_router(state);
    let addr = format!("0.0.0.0:{}", port);
    let listener = tokio::net::TcpListener::bind(&addr)
        .await
        .map_err(|e| crate::error::GpuMonError::ServiceUnavailable(format!("Failed to bind to port {}: {}", port, e)))?;

    tracing::info!("Web API server starting on http://localhost:{}", port);

    axum::serve(listener, app)
        .await
        .map_err(|e| crate::error::GpuMonError::ServiceUnavailable(format!("Server error: {}", e)))?;
...
async fn get_dashboard_info(State(state): State<ApiState>) -> Result<Json<DashboardInfo>, ApiError> {
    ...
    Ok(Json(DashboardInfo {
        gpu_count,
        database_path: "~/.local/share/gpumon/gpumon.db".to_string(),
        config_path: "~/.config/gpumon/config.toml".to_string(),
